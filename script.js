...
// https://restcountries.com/v2/name/United Kingdom
...
///////////////////////////////////////
// 260. Our First AJAX Call: XMLHttpRequest
...
const whereAmI = function (lat, lng) {
  // PART 1: Reverse geocoding (MUST use fetch + promises, no getJSON)
  fetch(
    `https://api.bigdatacloud.net/data/reverse-geocode-client?latitude=${lat}&longitude=${lng}`,
  )
    .then(res => {
    ...
    .then(data => {
      ...
      const city =
        data.city || data.locality || data.principalSubdivision || 'somewhere';
      const countryName = data.countryName;

      console.log(`You are in ${city}, ${countryName}`);

      // PART 2: Use result to fetch country data (use v2 to match your renderCountry)
...
Promise.resolve('abc1').then(x => console.log(x));
...
const whereAmI = function (lat, lng) {
  ...
  return fetch(
    ...
    .then(data => {
      ...
      console.log(`You are in ${city}, ${countryCode}`);

      // PART 2: Use result to fetch country data (use v2 to match your renderCountry)
      return fetch(`https://restcountries.com/v2/name/${countryCode}`);
    })
    .then(res => {
      if (!res.ok) throw new Error(`Country not found (${res.status})`);
      return res.json();
...
const createImage = function (imgPath) {
  return new Promise(function (resolve, reject) {
    ...
    img.addEventListener('load', function () {
      ...
      resolve(img); // fulfilled value is the img element
    });
